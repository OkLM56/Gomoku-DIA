...
def trouver_cases_adjacentes(grille):
    """Trouve toutes les cases adjacentes aux pions déjà placés."""
    adjacentes = set()
    ...
    for x in range(15):
        for y in range(15):
            if grille[x, y] != 0:
                for dx, dy in directions:
                    nx, ny = x + dx, y + dy
                    if 0 <= nx < 15 and 0 <= ny < 15 and grille[nx, ny] == 0:
                        adjacentes.add((nx, ny))
    return adjacentes
...
def minimax(grille, profondeur, maximiser, joueur, alpha, beta):
    ...
    meilleur_coup = None
    cases_adjacentes = trouver_cases_adjacentes(grille)
    if maximiser:
...
def trouver_meilleur_coup(grille, joueur):
    ...
    _, meilleur_coup = minimax(grille, 3, True, joueur, -float('inf'), float('inf'))  # Profondeur 3
